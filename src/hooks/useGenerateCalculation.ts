import { calculateCapsuleVolume, calculateCubeVolume, calculatePyramidVolume, calculateSphereVolume } from "@/maths/functionVolumens";
...
export const calculateIntegral = (shapeType:string,size:number, altura: number ) => {
    if (shapeType === 'Cubo') {
      ...
      return {
          ...
          aproximateResult: `V ≈ ${calculateCubeVolume(size,1000)}` 
      };
      ...
      return {
          ...
          result: `V = ${volume.toFixed(2)} unidades cúbicas`,
          aproximateResult: `V ≈ ${calculateSphereVolume(size,1000)}`,
      ...
      return {
        ...
        aproximateResult: `V ≈ ${calculatePyramidVolume(baseLength,height,1000)}`
      };
      ...
      return {
        ...
        formula: `V = ∫∫∫ dxdydz = π r² h + 4/3 π r³`,
        calculation: `V = (π × (${radius.toFixed(2)})² × ${height.toFixed(2)}) + (4/3 × π × (${radius.toFixed(2)})³)`,
        ...
        aproximateResult: `V ≈ ${calculateCapsuleVolume(radius, height, 1000)} unidades cúbicas`
      };
    }else {
      return {
        formula: `V = ∫∫∫ dxdydz`,
